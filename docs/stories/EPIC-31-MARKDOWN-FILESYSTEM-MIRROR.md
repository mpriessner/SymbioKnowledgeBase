# Epic 31: Markdown Filesystem Mirror — Bidirectional Sync
...
### SKB-31.1: Round-Trip Fidelity & Format Specification — 3 points, High
...
### SKB-31.2: Filesystem Mirror — Initial Sync & Folder Structure — 8 points, High
...
**Depends on:** SKB-31.1 (serialization must be deterministic first)

---

### SKB-31.3: DB-to-Filesystem Live Sync — 5 points, High
...
### SKB-31.4: Filesystem-to-DB Live Sync — 8 points, High
...
### SKB-31.5: File Attachments & Relative Linking — 5 points, Medium
...
### SKB-31.6: Agent Filesystem API & MCP Tools — 8 points, High
...
### SKB-31.7: Conflict Resolution & Sync Health — 3 points, Medium
...
|-------|-----------|-------------------|-----------|
| 31.1 | Round-trip for each of 27 block types; frontmatter serialization/deserialization | Full document round-trip with mixed content; edge cases (empty pages, huge pages) | N/A |
| 31.2 | Folder structure generation; slug naming; parent→folder mapping | Full DB export → verify folder structure matches page tree | CLI command produces correct mirror |
| 31.3 | Sync lock mechanism; file write debouncing | Save page → .md file updates; rename page → file moves; delete page → file deleted | Edit in browser → verify .md changes |
| 31.4 | File watcher event handling; .md parsing; new file detection | Edit .md → DB updates; create .md → page appears; delete .md → page removed | Agent edits file → browser shows change |
| 31.5 | Relative path generation; link updating on move | Place image in assets/ → referenced in page; move page → links update | Upload image → visible in editor and .md |
| 31.6 | MCP tool parameter validation; response formatting | Browse → read → edit → verify round-trip via MCP tools | Agent creates page via MCP → visible in app |
| 31.7 | Conflict detection logic; .conflict file creation | Simultaneous edit → conflict file created; sync recovers | Stress test: rapid edits from both sides |
