...
6. The apply — page body, `DocumentVersion`, `Claim`/`ClaimEvidence`, audit, ledger — commits in **one transaction or none**; a quote-gate/FK/version failure leaves no live uncited page. **`Source`+`SourceChunk` are an idempotent pre-step, explicitly OUTSIDE this transaction** (W81-A1 Codex R1 — Source is immutable/dedup-idempotent, so an orphan Source from a rolled-back apply is harmless GC-able raw data; this resolves the earlier contradiction where AC6 and §2 disagreed on whether Source is inside the tx). Retolerant to retry via the `@@unique([claimId, chunkId, quoteSha256])` idempotency key.
